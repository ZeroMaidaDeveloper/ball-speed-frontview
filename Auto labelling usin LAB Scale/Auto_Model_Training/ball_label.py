"""
ball_autolabel.py  -  MOG2+Lab detection + YOLO person-mask filter + manual click
...
# =====================================================================
# NEW: find contour from manual click
# =====================================================================

def find_contour_bbox_from_click(frame, point, motion_mask, a_channel,
                                 search_radius=40, min_area=5, max_area=800):
    """
    Given a click point, search the motion mask and LAB a‑channel in a small
    ...
    # Crop motion mask and a‑channel
    roi_motion = motion_mask[y1:y2, x1:x2]
    roi_a = a_channel[y1:y2, x1:x2]

    # Optional: combine with LAB red threshold to refine
    # But we can just use motion mask first.
    # Find contours in the motion ROI
    contours, _ = cv2.findContours(roi_motion, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        # Fallback: use a‑channel threshold
        # but we'll just return None, so fallback to fixed box later.
        return None
    ...
    best_dist = float('inf')
    for cnt in contours:
        # Check if point is inside contour (using point-in-polygon)
        # Convert contour coordinates back to full image
        ...
        if dist >= 0:  # inside or on edge
            ...
            if area < min_area or area > max_area:
                continue
            # Also check aspect ratio to avoid long shapes
            aspect = max(w, h) / max(1, min(w, h))
            if aspect > 3.0:
                continue
            # Prefer tighter box – we can just take the first valid one,
            # but we take the one with smallest area (likely the ball)
            if area < best_dist:  # using area as proxy for distance
                best_dist = area
                best_bbox = (x, y, w, h)
        else:
            # If point is outside, check if contour is very close
            # Use distance from point to contour
            d = abs(dist)
            if d < 10 and d < best_dist:
                # also check area and aspect
                x, y, w, h = cv2.boundingRect(cnt_full)
                area = w * h
                if area < min_area or area > max_area:
                    continue
    ...
    # If still none, try using a‑channel threshold to find red blob
    # Threshold a-channel (red is high)
    _, thresh = cv2.threshold(roi_a, 135, 255, cv2.THRESH_BINARY)
    # Morph to clean
...
# =====================================================================
# MAIN (with manual contour extraction and UI freeze fix)
# =====================================================================
...
def main():
    ...
    win = "AUTO-LABEL  S save N skip Q quit | click=label  right-drag=pan  i/o zoom  0 reset"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)
    ...
    while True:
        ...
        if manual_pt is not None:
            ...
            if manual_bbox is None:
                bbox = find_contour_bbox_from_click(frame, manual_pt, motion, a_channel,
        ...
        render = render_lab(frame) if SAVE_IMAGE == "lab" else frame
        label_box = None
        if ball is not None:
            if manual_pt is not None:
